...
Before submitting a new issue or PR, check if it already exists in the [Issues](https://github.com/varunmara/gems-of-india/issues) or [Pull Requests](https://github.com/varunmara/gems-of-india/pulls).
...
Avoid posting third-party links (e.g., Discord threads or Linear tickets) without context. A GitHub issue or PR should stand on its own—reviewers shouldn’t have to chase information across multiple tools to understand the context.
...
If the task originated from a private conversation (e.g., Discord), take a moment to extract the relevant details and include them in the GitHub issue or PR. Avoid sharing sensitive information, but make sure important reasoning is captured.
...
Kindly check the section `Local Development` of [DEPLOYMENT.md](./docs/DEPLOYMENT.md) for more info on how to set up the development environment.

## Building

Kindly follow the sections `Docker Setup (Development)` and `Production Deployment` of [DEPLOYMENT.md](./docs/DEPLOYMENT.md) file for building the project.
